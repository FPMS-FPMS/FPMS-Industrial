...
stateDiagram-v2
    direction LR
    [*] --> Hover
    Hover --> Transition: climb complete
    Transition --> Cruise: reach airspeed
    Cruise --> Transition: approach
    Transition --> Hover: descend
    Hover --> [*]: land
```

In **Hover**, all three rotors point up (T1 up · T2 up · M3 up). In **Cruise**, the two front
rotors tilt forward and the tail motor shuts off (T1 fwd · T2 fwd · M3 off), and the wing carries
the weight.
...
Full part numbers and prices: **[BOM.md](BOM.md)**
